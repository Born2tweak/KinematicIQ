...
| **Carry** (farmer's/suitcase) | sustained (gait + transitions) | trunk/stride strategy under offset load | **the star question** — posture endurance over time-under-task | lean toward/away from load (asymmetry is the *intent* in unilateral carries) | translating athlete vs static camera is the protocol problem; see §9b |
...
## 9b. Carry paper test — second pass (pre-Phase-2 interface gate)

Method: same as §9, applied to the loaded carry (farmer's / suitcase). Chosen deliberately as the *hard* exam: the hinge is structurally a squat (cyclic, descent-led, knee-adjacent); the carry has **no reps, no descent, no bottom, and no monotonic progress signal** — it is a steady-state endurance of posture during translation, bracketed by pickup and setdown. If the interfaces survive carry on paper, the "movement-generic" claim is earned; if they only survive hinge, it was never tested. No carry code ships now; findings are interface requirements that must land **before the Phase 2 type freeze**, because each is cheap to adopt in a fresh type and expensive to retrofit into a frozen one.

**What holds with zero change:** verdict-or-abstain semantics; the observation → evidence → interpretation → verdict → recommendation chain and its type separation; abstention reasons; the seven-field label contract (§7) including decision consequence and counterfactual; the metric rule (§5); confidence gating; protocol separation and (question, protocol) validation scoping (P5); the allowed/forbidden conclusion lists (§6); the five evidence categories (`temporal`, `laterality`, `variability`, `capture-quality` apply directly; `kinematic-geometry` at defined *moments* generalizes to defined *windows*). The deep reasoning chain is genuinely movement-generic. What carry breaks is the **addressing scheme, the progress abstraction, and the assumed universality of one vocabulary** — all shallow-layer assumptions, all fixable in Phase 2 type design.

**Finding C1 — phases are not the universal address; evidence addresses a `MovementSegment`.** §3.3 makes phases "the addressing scheme for all evidence," and every phase family listed is event-rich (cyclic, ballistic, gait strides). A carry's phase structure is trivial (pickup / carry / setdown) and its interesting evidence lives *inside* the featureless carry phase, addressed by time-under-task ("trunk lean increased over the final third of the carry window"), not by phase-progress-%. → Requirement: the evidence address type is a `MovementSegment` union — rep, stride window, time window — not a phase + rep pair. `Observation.repNumbers` (§3.9) becomes segment references; `variability` (§3.7) disperses across segments, not only reps. Cyclic movements lose nothing (a rep is one segment kind); carry and gait become expressible.

**Finding C2 — the progress domain is profile-defined, not always a within-rep scalar.** §9 Finding 1 generalized the progress signal but kept the assumption that *some* monotonic scalar rises through each rep. A carry has none: its progress is elapsed time or distance fraction of the task. → Requirement: `MovementProfile.progressSignal` selects a **normalization domain** (rep-progress % | task-time fraction | distance fraction), and `completionStandard` is a predicate (or conjunction of predicates) over segments — "held the implement and kept walking through the observed window" is a conjunction, not a band on one scalar. Squat's depth band is the degenerate single-predicate case, so v1 code is unaffected.

**Finding C3 — vocabulary universality is per-question, not global; declare it, don't assume it.** §9 Finding 2 claimed "vocabularies are universal; interpretation maps are per-movement," and for `load-symmetry` carry confirms it beautifully — centered / shift-left / shift-right transfers intact, and the unilateral carry is the second case (after hinge's hip-dominance) where a "deviation" label is the *intent*, handled entirely by the per-movement interpretation map. But `strategy-selection`'s squat vocabulary (hip-dominant / knee-dominant / balanced) is meaningless for a carry, whose strategy axes are trunk-lean strategy and stride adjustment under load. → Requirement: `MovementQuestion` declares whether its vocabulary is **universal** (load-symmetry, movement-completion) or **pattern-scoped** (strategy-selection, resolved through the active `MovementProfile`). Both kinds still obey the §7 contract per label; what changes is where the label set lives.

**Finding C4 — organization evidence generators are a per-question *family*, not one parameterized generator.** §9 Finding 3's reuse claim holds within the cyclic family (hinge reuses squat's trunk-vs-progress machinery with new parameters). For carry, the *shape* of the organization evidence changes: the question is no longer "how did posture trace through a large ROM" but "how far did segments that should be still excurse over time" — drift and excursion statistics over windows, against a self-referenced early-window baseline. → Requirement: Phase 3 plans `postureOrganization` as a family of generators (trajectory-through-progress for cyclic; excursion-over-window for sustained/gait) sharing observation utilities, selected by the profile's segmentation kind. Expect the same for `strategy-selection`. Interfaces should expose generator selection per (question, kind), not a single generator per question.

**Finding C5 — a translating athlete breaks the static-framing assumption; protocols need observed-window coverage.** Every current protocol assumes the athlete stays in frame. A carry translates: side-view framing at 3–4 m yields ~2–3 seconds of usable frames; the realistic protocols are walk-toward-camera, walk-away-and-return, or wide/high framing — each with different landmark-confidence profiles. The protocol abstraction (P5) absorbs this, but only if `CaptureProtocol` carries a notion of **task coverage**: what fraction of the task the camera actually observed, feeding `capture-quality` evidence, with every carry verdict scoped to the observed window ("in the observed 8 seconds of the carry…"). This also sharpens a general rule the cyclic movements never forced: **completion verdicts are claims about the observed window, never about the unobserved task.**

**Verdict on the interfaces:** the reasoning chain holds; the Phase 2 types must adopt C1 (segment addressing), C2 (progress domain + predicate completion), and C3 (declared vocabulary scope) **before freezing**, since all three touch types that Phase 3 builds on. C4 and C5 are Phase 3/protocol-design requirements and do not block the freeze. Re-run both paper tests (hinge + carry) against the frozen interfaces at the end of Phase 2; carry implementation itself remains far down the roadmap (§8) — this test exists to keep the shared types honest, not to schedule carry.
...
| Coach question | `MovementQuestion`, `QuestionId` (`analysis/questions/`) — vocabulary scope declared universal vs pattern-scoped (Finding C3) | Phase 2 |
| Completion standard | `MovementProfile.completionStandard` — predicate(s) over segments, scoped to the observed window (Findings C2, C5) | Phase 2 |
| Progress signal | `MovementProfile.progressSignal` (Finding 1) — selects a normalization domain, not only a scalar (Finding C2) | Phase 2 |
| Evidence address | `MovementSegment` union (rep / stride window / time window) on `Observation` (Finding C1) | Phase 2 |
