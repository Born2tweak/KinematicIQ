...
The repository currently contains no lunge implementation and calls the research track “inline lunge.” Evidence review shows that the proposed stepping task is not the fixed-stance FMS inline lunge. The owner must approve the identity before any participant capture or external acquisition. This specification does not authorize either action.
