...
The supplied research prompt says Phase 4 must continue implementing inline lunge and refers to a live lunge implementation. That premise conflicts with current repository authority. `docs/research/INLINE_LUNGE_PROTOCOL_RESEARCH.md` says M73 is research-only and not approved for implementation; `docs/research/INDEX.md` says it authorizes neither acquisition nor implementation; `docs/implementation/NEXT_EXECUTION_PACKAGE.md` makes M78 a future data-and-label gate; and no lunge protocol, runtime, metric, or quality gate exists under `web/src`. This review follows the live repository: it does not authorize or specify code implementation. Reversible research fixtures become appropriate only after the roadmap's data/label and owner-approval gates are explicitly opened.
...
Under current repository authority, the engineering implication is **no lunge production implementation**. The next action remains the M78 data-and-label gate and a named owner decision. This review supplies contracts that a later approved milestone can translate into code.
