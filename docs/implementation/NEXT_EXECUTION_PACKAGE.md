...
> **Expanded-10 Revision 4 authority (2026-07-21):** KQ-001 through KQ-014 are complete with repository-bound evidence. KQ-015 is the sole remaining Phase A node and next scheduled milestone; KQ-016 and KQ-017 are also ready as bounded Phase B corpus-freeze work. The deterministic checkpoint now reports completed IDs, evidence and verification, subject commits, protocol-state changes, research, replans, release dispositions, blockers, active work, and the next executable range without granting release or deployment authority. KQ-015 clean-clone integration remains before the Phase A gate can pass. Squat remains the only available protocol; Forward Lunge and every additional movement remain unavailable until their preregistered release gates produce `GATE_PASS`.
