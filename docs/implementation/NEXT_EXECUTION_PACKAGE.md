...
> **Expanded-10 Revision 4 authority (2026-07-21):** KQ-001 through KQ-004 are complete with repository-bound evidence. The immediate dependency-ready Phase A frontier is KQ-005 (history import), KQ-007 (execution policy), and KQ-010 (traceability validator); KQ-016 and KQ-017 are also ready as bounded Phase B corpus-freeze work. The schema kernel validates dependencies, typed gates, retries, evidence provenance, command bindings, and commit policy without making the mutable whole registry part of each milestone's immutable evidence identity. KQ-005 through KQ-015 remain the committed Phase A queue before scheduler authority is granted. Squat remains the only available protocol; Forward Lunge and every additional movement remain unavailable until their preregistered release gates produce `GATE_PASS`.
