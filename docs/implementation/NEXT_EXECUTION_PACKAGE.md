...
> **Expanded-10 Revision 4 authority (2026-07-21):** KQ-001 through KQ-008 are complete with repository-bound evidence. The immediate dependency-ready Phase A frontier is KQ-009 (status compiler), KQ-010 (traceability validator), and KQ-012 (research-question workflow); KQ-016 and KQ-017 are also ready as bounded Phase B corpus-freeze work. Typed inflection events now deterministically select only unresolved dependency descendants, while completed evidence remains immutable and the real append-only trigger log remains empty until an actual event occurs. KQ-009 through KQ-015 remain the committed Phase A queue before scheduler authority is granted. Squat remains the only available protocol; Forward Lunge and every additional movement remain unavailable until their preregistered release gates produce `GATE_PASS`.
